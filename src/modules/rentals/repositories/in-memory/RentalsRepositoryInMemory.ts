...
class RentalsRepositoryInMemory implements IRentalsRepository {
  ...
  async findOpenRentalByCar(carId: string): Promise<Rental | undefined> {
    return this.rentals.find(
      rental => rental.carId === carId && !rental.finishAt,
    );
  }

  async findOpenRentalByUser(userId: string): Promise<Rental | undefined> {
    return this.rentals.find(
      rental => rental.userId === userId && !rental.finishAt,
    );
  }
  ...
  async findOpenRentalByIdAndUserId(
    id: string,
    userId: string,
  ): Promise<Rental | undefined> {
    return this.rentals.find(
      rental => rental.id === id && rental.userId === userId,
    );
  }
