...
class CreateRentalUseCase {
  static minRentalReturnDateInHours = 24;
  ...
  async execute({
    ...
    const isOpenRentalToUser = await this.rentalsRepository.findOpenRentalByUser(
      userId,
    );
    ...
    const diffStartAtDateAndExpectReturnDateInHours = this.dateProvider.compareInHours(
      startAt,
      expectReturnDate,
    );
    if (
      diffStartAtDateAndExpectReturnDateInHours <
      CreateRentalUseCase.minRentalReturnDateInHours
    ) {
